...
const AdminDashboard = () => {
    ...
    const exportToCSV = async () => {
        ...
        const csvRows = certifications.map(cert => [
            ...
            escapeCSV(new Date(cert.issueDate).toLocaleDateString()),
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-gray-100">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            ...
                            <button onClick={() => setSelectedCert(null)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-xl transition"><X className="w-6 h-6" /></button>
                        </div>
                        <div className="p-6 space-y-6">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Uploaded By</p>
                                    <p className="font-semibold text-gray-900">{selectedCert.user?.name}</p>
                                    <p className="text-sm text-gray-600 capitalize">{selectedCert.user?.role} • {selectedCert.user?.rollNo || selectedCert.user?.employeeId}</p>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Status</p>
                                    <span className={`px-3 py-1 inline-flex text-sm leading-5 font-bold rounded-full shadow-sm w-fit ${selectedCert.status?.toLowerCase() === 'approved' ? 'bg-emerald-100 text-emerald-800' : selectedCert.status?.toLowerCase() === 'rejected' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'} capitalize`}>{selectedCert.status}</span>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 col-span-2">
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Certificate Title</p>
                                    <p className="font-bold text-lg text-gray-900">{selectedCert.title}</p>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Issuing Organization</p>
                                    <p className="font-semibold text-gray-900">{selectedCert.issuingOrganization}</p>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Issue Date</p>
                                    <p className="font-semibold text-gray-900">{new Date(selectedCert.issueDate).toLocaleDateString()}</p>
                                </div>
                            </div>
                            
                            {selectedCert.ocrResult && selectedCert.ocrResult.processed && !selectedCert.ocrResult.isMatch && (
                                <div className="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-xl">
                                    <div className="flex">
                                        <AlertTriangle className="h-5 w-5 text-rose-500" />
                                        <div className="ml-3">
                                            <h3 className="text-sm font-bold text-rose-800">AI Flagged Mismatch</h3>
                                            <div className="mt-2 text-sm text-rose-700">
                                                <ul className="list-disc pl-5 space-y-1">
                                                    {selectedCert.ocrResult.warnings?.map((w, i) => <li key={i}>{w}</li>)}
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div>
                                ...
                                <a href={selectedCert.fileUrl} target="_blank" rel="noreferrer" className="block w-full text-center py-3 bg-indigo-50 border-2 border-dashed border-indigo-200 rounded-xl text-indigo-600 font-bold hover:bg-indigo-100 hover:border-indigo-300 transition">
                                    Click here to open full document in new tab
                                </a>
                                {selectedCert.fileUrl && (selectedCert.fileUrl.endsWith('.png') || selectedCert.fileUrl.endsWith('.jpg') || selectedCert.fileUrl.endsWith('.jpeg')) && (
                                    <div className="mt-4 border rounded-xl overflow-hidden shadow-sm">
                                        <img src={selectedCert.fileUrl} alt="Certificate" className="w-full h-auto object-cover max-h-96" />
                                    </div>
                                )}
                            </div>
                        </div>
